...
public class Board {
    ...
    public Board(int nPlayers, int boardSize) {
        ...
        this.running = true;
        for (int i = 0; i < nPlayers; i++) players.add(i, new Player(i + 1));
    ...
    public String toString() {
        ...
        for (int i = 1; i <= players.size(); i++) {
            Player p = null;
            for (Player player : players) if (player.getLabel() == i) p = player;
            sb.append("player"+p.getLabel()+": ");
            for (int j = 0; j < this.boardSize; j++) {
                if(j == p.getPos()) sb.append(p.getLabel());
                else sb.append('.');
            }
            sb.append("\n");
        }
        ...
        for (int i = 0; i < this.boardSize; i++) {
            if (traplist.contains(i)) sb.append("x");
            else sb.append(".");
        }
        return sb.toString();
    }
    ...
    public void rollDice(int value) {
        if (this.running) {
            ...
            if (p.isFree()) {
                ...
                if (value > possible) newPos = this.boardSize - 1;
                p.setPos(newPos);
